    ...
    async def send(
        ...
        **meta,
        ...
        if not webhook and self._settings is not None:
            webhook = getattr(self._settings, "wecom_webhook", None)
        if not webhook:
            logger.info(
                "WeCom mock: tenant=%s title=%s severity=%s",
                tenant_id, title, severity,
            )
            return f"mock-wecom-{secrets.token_hex(4)}"
        # 真实发送：阶段五实现 POST 到 webhook
        logger.info("WeCom webhook 发送: %s", webhook[:30] + "...")
        return f"mock-wecom-{secrets.token_hex(4)}"
